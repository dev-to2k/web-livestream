...
const useWebRTC = (socket, roomId) => {
  ...
  const createPeerConnection = useCallback(
    async (mediaStream) => {
      ...
      console.log('🔵 WebRTC: Creating peer connection with RTC config:', RTC_CONFIG);
  ...
  const handleIceCandidate = useCallback(async (candidate) => {
    ...
    try {
      ...
      console.warn('🟡 WebRTC: Failed to add ICE candidate (non-fatal):', error);
    }
  }, []);
  ...
  const startStream = useCallback(
    async (localVideoRef) => {
      ...
      const attemptStream = async (constraints, attemptNumber = 1) => {
        ...
        try {
          ...
          console.error(`🔴 useWebRTC: getUserMedia failed on attempt ${attemptNumber}:`, getUserMediaError);
        ...
        try {
          ...
          console.error('🔴 useWebRTC: Failed to create peer connection:', peerError);
          // Clean up media stream if peer connection fails
          mediaStream.getTracks().forEach(track => {
            track.stop();
            console.log(`🔵 useWebRTC: Stopped ${track.kind} track due to peer connection failure`);
          });
          throw peerError;
        }
      ...
      try {
        ...
        if (retryStrategy.shouldRetry(error)) {
          ...
          if (nextAttempt) {
            console.log(`🟡 useWebRTC: Retrying with ${nextAttempt.quality} quality after ${nextAttempt.delay}ms`);
            ...
            try {
              ...
              if (finalAttempt) {
                console.log(`🟡 useWebRTC: Final attempt with ${finalAttempt.quality} quality after ${finalAttempt.delay}ms`);
    ...
    [createPeerConnection, getMediaConstraints]
  );
  ...
  const stopStream = useCallback(() => {
    ...
    if (localStreamRef.current) {
      localStreamRef.current.getTracks().forEach((track) => {
        track.stop();
        console.log(`🔵 useWebRTC: Stopped ${track.kind} track (${track.label})`);
      });
  ...
  return {
    ...
    isConnecting: streamState === 'requesting' || connectionState === 'connecting',
