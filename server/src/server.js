...
const { Room, User, Message, ConnectionState } = require('./src/models');
...
class ScalableStreamingServer {
  ...
  async initialize() {
    try {
      console.log(`🚀 SERVER: Initializing Scalable Streaming Server (${SERVER_ID})`);
  ...
  setupSocketIO() {
    ...
    if (redisManager.isConnected) {
      try {
        ...
        this.io.adapter(createAdapter(redisManager.pubClient, redisManager.subClient));
        console.log('✅ SOCKET.IO: Redis adapter configured');
      } catch (error) {
        console.warn('⚠️ SOCKET.IO: Redis adapter not available, running in single-server mode');
      }
    }
  ...
  setupSocketHandlers() {
    this.io.on('connection', async (socket) => {
      ...
      console.log(`🔌 SOCKET: User connected - ${socket.id} (Total: ${this.stats.connectionsCount})`);
      
      // Connection limit check
      if (this.stats.connectionsCount > MAX_CONNECTIONS) {
        console.warn(`⚠️ SOCKET: Connection limit reached, rejecting ${socket.id}`);
        socket.emit('error', { code: 'CONNECTION_LIMIT', message: 'Server at capacity' });
        socket.disconnect();
        return;
      }
  ...
  registerSocketEvents(socket) {
    ...
    socket.on('update-auto-accept', (data) => this.handleUpdateAutoAccept(socket, data));
    
    // WebRTC signaling
    socket.on('offer', (data) => this.handleOffer(socket, data));
    socket.on('answer', (data) => this.handleAnswer(socket, data));
    socket.on('ice-candidate', (data) => this.handleIceCandidate(socket, data));
    socket.on('connection-health', (data) => this.handleConnectionHealth(socket, data));
    
    // Chat messaging
    socket.on('chat-message', (data) => this.handleChatMessage(socket, data));
    
    // Stream control
    socket.on('stream-started', (data) => this.handleStreamStarted(socket, data));
    socket.on('stream-ended', (data) => this.handleStreamEnded(socket, data));
    
    // Performance monitoring
    socket.on('performance-metrics', (data) => this.handlePerformanceMetrics(socket, data));
  }

  async handleJoinRoom(socket, { roomId, username, isStreamer }) {
    try {
      console.log(`🏠 ROOM: ${username} joining room ${roomId} as ${isStreamer ? 'streamer' : 'viewer'}`);
      ...
      // Get or create room
      let room = await this.getOrCreateRoom(roomId, isStreamer ? { id: socket.id, username } : null);
      ...
      // Update metrics
      this.metrics.incrementCounter('room_joins_total', { type: isStreamer ? 'streamer' : 'viewer' });
  ...
  checkRateLimit(socket, username, isStreamer) {
    const userType = isStreamer ? 'streamer' : (username ? 'viewer' : 'anonymous');
  ...
  async getOrCreateRoom(roomId, streamer = null) {
    ...
    room = await Room.findOne({ roomId }) || await this.createNewRoom(roomId, streamer);
  ...
  async acceptViewer(socket, room, username) {
    ...
    console.log(`👁️ VIEWER: ${username} joined room ${room.roomId} (${room.streamStats.currentViewers} viewers)`);
  }
  ...
  async start() {
    ...
    this.server.listen(PORT, () => {
      console.log(`🚀 SERVER: Scalable Streaming Server running on port ${PORT}`);
